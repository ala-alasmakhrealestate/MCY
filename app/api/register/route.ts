import { google } from "googleapis";
import {NextRequest} from "next/server";

export async function POST(req:NextRequest) {
    try {
        const form = await req.json(); // get JSON body
        const nowDate = new Date();

        const pad = (n: number | string) => n.toString().padStart(2, "0");
        const createdDate = `${nowDate.getFullYear()}-${pad(nowDate.getMonth() + 1)}-${pad(nowDate.getDate())} ${pad(nowDate.getHours())}:${pad(nowDate.getMinutes())}`;

        const required = ["fullName", "offer", "movingDate", "phone"];
        for (const f of required) {
            if (!form[f]) {
                return new Response(JSON.stringify({ error: `${f} is required` }), { status: 400 });
            }
        }

        const auth = new google.auth.GoogleAuth({
            credentials: {
                type: "service_account",
                project_id: "flawless-star-407410",
                client_email: "sheet-writer@flawless-star-407410.iam.gserviceaccount.com",
                private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
                    .replace(/\\n/g, "\n"),
            },
            scopes: ["https://www.googleapis.com/auth/spreadsheets"],
        });

        const sheets = google.sheets({ version: "v4", auth });

        await sheets.spreadsheets.values.append({
            spreadsheetId: "1khP2lqrKl2oUuCA39hR0TFcM1A6jrWVi8W9GOnQN_bU",
            range: "MCY!A:E",
            valueInputOption: "USER_ENTERED",
            requestBody: {
                values: [[
                    createdDate,
                    form.fullName,
                    form.phone,
                    form.offer,
                    form.movingDate,
                    "Landing Page"
                ]]
            }
        });

        return new Response(JSON.stringify({ success: true, message: "Registration successful" }), { status: 200 });
    } catch (err) {
        return new Response(JSON.stringify({ success: false, error: (err as Error).message || err }), { status: 500 });
    }
}
